$(document).ready(function(){
    ...
    $('.nav-main a[href="'+activeurl+'"]').parent('li').addClass('active');
});
...
function loadService(pg_id=0, _page=0, resizeWidth=0) {
    ...
    {
        btn_appointment = `<a class="btn btn-primary mb-15 btn_make_appointment" href="/book">Make an appointment</a>`;
    }
    var btn_callnow = `<a class="btn btn-primary mb-15" style="margin-left:15px;" href="tel:` + company_phone + `"><i class="fa fa-phone"></i>Call now</a>`;
...
function pushHtmlTime(input_date,type) {
    ...
    // Get time by format
    var str1 = splitDate[0] == 0 ? d.getDate() : (splitDate[0] == 1 ? d.getMonth() + 1 : d.getFullYear());
    var str2 = splitDate[1] == 0 ? d.getDate() : (splitDate[1] == 1 ? d.getMonth() + 1 : d.getFullYear());
    var str3 = splitDate[2] == 0 ? d.getDate() : (splitDate[2] == 1 ? d.getMonth() + 1 : d.getFullYear());
    // Date check
    var checkdate = (str1 < 10 ? "0"+str1 : str1) +"/"+(str2 < 10 ? "0"+str2 : str2)+"/"+(str3 < 10 ? "0"+str3 : str3);
    // Time get
    var currHours = d.getHours() + parseInt(beforeTime);
    var currMinutes = d.getMinutes();

    var defaultMorning = JSON.parse(timeMorning);
    var defaultAfternoon = JSON.parse(timeAfternoon);
    var htmlMorning = "";
    var htmlAfternoon = "";
    // console.log(input_date ,checkdate);
    var checkmorning = false;
    var checkafternoon = false;
    var suffixMorning = "";
    var suffixAfternoon = "";
    ...
    {
        // Like Date
        var html_expired = " (Overdue scheduled)";
        var style_option = " style='color: #909090;' ";

        // Html morning
        for(var x in defaultMorning)
        {
            var checktime = defaultMorning[x].split(":");
            if(parseInt(checktime[0]) < currHours)
            {
            ...
            }else if(parseInt(checktime[0]) == currHours)
            {
                if(parseInt(checktime[1]) < currMinutes)
                {
                    htmlMorning += type == "html" ? `<li><span onclick="call_notify('Notification', 'You can not booking in this time, please choose another.')">`+defaultMorning[x]+suffixMorning+`</span></li>` : `<option value="`+defaultMorning[x]+`" disabled`+style_option+`>`+defaultMorning[x]+suffixMorning+html_expired+`</option>`;
                }else
                {
        ...
        // Html Afternoon
        for(var x in defaultAfternoon)
        {
            var checktime = defaultAfternoon[x].split(":");
            var timeShow = hoursTimeFormat == 12 ? (checktime[0]>12 ? ((checktime[0]%12)+":"+ checktime[1] + suffixAfternoon) : (checktime[0]+":"+ checktime[1] + suffixAfternoon)) : defaultAfternoon[x];
            if(parseInt(checktime[0]) < currHours)
            {
    ...
    $(".databooktime .timemorning").html(htmlMorning);
    $(".databooktime .timeafternoon").html(htmlAfternoon);
}
    ...
    $.ajax({
        ...
        success: function(html)
        {
            ...
            if(obj.amount)
            {
                ...
                {
                    $(".list-row tbody").html('<tr><td colspan="6"><p>Cart empty...</p></td></tr>');
                }
...
(function($) {
    ...
    var today = new Date();
    var future = new Date();
    var set_date = parseInt(beforeDay) > 0 ? new Date(future.setDate(today.getDate()+beforeDay)) : (checktimebooking == 1 ? new Date(future.setDate(today.getDate()+1)) : new Date());
